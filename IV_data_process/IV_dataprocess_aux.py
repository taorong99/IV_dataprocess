...
def create_table(filepaths: list, fit_results: list[np.ndarray], curve_types: list, Rsg_results, array_params, save_table=False):
    ...
    for filepath, fit_result, curve_type,Rsg_result, array_param in zip(filepaths, fit_results, curve_types, Rsg_results, array_params):
        ...
        if has_JJa:
            data.append([filepath.split('/')[-1].split('.')[0], f'{Ic_1*1e6:.1f}, {Ic_2*1e6:.1f}', f'{R_fitp:.2f}, {R_fitm:.2f}', IcR_p+', '+IcR_m, f'{intercept_p*1e3:.2f}, {intercept_m*1e3:.2f}', (Rsg_p+', '+Rsg_m), num_JJ, Vg_optimal, curve_type])
        else:
            data.append([filepath.split('/')[-1].split('.')[0], f'{Ic_1*1e6:.1f}, {Ic_2*1e6:.1f}', f'{R_fitp:.2f}, {R_fitm:.2f}', IcR_p+', '+IcR_m, f'{intercept_p*1e3:.2f}, {intercept_m*1e3:.2f}', (Rsg_p+', '+Rsg_m), curve_type])
