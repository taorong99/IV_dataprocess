"""
Created on 2024-12-27 12:48:43.
    ...
    def plot_figure(self):
        ...
        for k in range(len(self.iv_list)):
            if self.plot_check_list[k].get() == 1:
                try:
                    I_max = max(I_max, max(self.iv_list[k].I_data))
                    V_max = max(V_max, max(self.iv_list[k].V_data))
                except:
                    continue
        suffix_I, multiplier_I = self.iv_list[0].number_suffix(I_max)
        suffix_V, multiplier_V = self.iv_list[0].number_suffix(V_max)
        ...
        for k in range(len(self.iv_list)):
            if self.plot_check_list[k].get() == 1:
                ...
                try:
                    ax.plot(
                        iv.V_data / multiplier_V,
                        iv.I_data / multiplier_I,
        ...
        ax.set_xlabel(f'V({suffix_V}V)')
        ax.set_ylabel(f'I({suffix_I}A)')
        ax.grid()
